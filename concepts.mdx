...
<Accordion title="Document">
  A document is the top-level container for a single SOP, playbook, guide, or handbook. Every document has:

  - **Title.** The name displayed in the dashboard and payloads.
  - **Type.** The kind of document: `sop`, `guide`, `playbook`, or `handbook`. You select the type when you create the document. It determines the structure and instructions the AI uses to generate content.
...
<Accordion title="Share token">
  ...
  ```
  app.docinject.com/share/{org_slug}/{token}
  ```
