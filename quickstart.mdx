...
description: "Create your DocInject account, build and publish your first SOP, then share it via share link or route events to an automation platform in minutes."
---

This guide walks you from a brand-new account to a published, deployed document. By the end, you'll have created a document, added structure to it, published it, and either shared it via a shareable link or wired it to a webhook.
...
<Steps>
  ...
  <Step title="Create your first document">
    From the dashboard, click **New Document**. First, choose the document type:

    | Type | Use it for |
    | --- | --- |
    | **SOP** | Standard operating procedures with structured sections like Purpose, Scope, and Company Procedures |
    | **Guide** | Educational walkthroughs that explain concepts and how-tos |
    | **Playbook** | Scenario-based documents with recommended actions for each situation |
    | **Handbook** | Reference material and policies organized for ongoing consultation |

    Then choose how to create it:
    ...
    For AI-assisted modes (Notes, Audio, Video, Document), DocInject generates a draft outline and asks a few clarifying questions before creating the document. Review the draft outline and fill in any answers.

    Give your document a **title**, **version**, and optionally a **department** before submitting.

    <Tip>
      If you have an existing document in another tool, the **Document** mode is the fastest way to import it. Upload the file and DocInject will extract and structure the content for you.
    </Tip>
  </Step>
  ...
  <Step title="Share or deploy">
    ...
    **Share via a link**
    ...
    Open the document and generate a share token. This creates a public URL at `app.docinject.com/share/{org_slug}/{token}` that anyone can view without a DocInject account. Copy the URL from the **Shareable links** section of your dashboard.
